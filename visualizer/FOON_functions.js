	...
	text.append("text")
		...
		.each(function (d) {
			...
			for (var i = 0; i < lines.length; i++) {
				d3.select(this)
					.append("tspan")
					.attr("dy", function (d) { if (i == 0) return square_length+2; return separation+2; })
					.attr("x", textX)
					...
					.style("font-size", function (d) { if (d.type == "1" && (lines[i].includes(":") || lines[i].includes("<"))) return "1.5em"; return "1.6em" }) // -- original!
					//.style("font-size", "0em")
					.text(function(d){
